"""
01_discover.py
TrashLab Texas Hauler Discovery — v6
...
- Keeps Austin as a guarded source: if its markup cannot be parsed, it fails
  loudly without contaminating the candidate file.
...
def parse_austin(source: Source, raw: bytes) -> list[dict]:
    """
    Austin's current page visually renders the URO hauler list, but its HTML
    structure can change. Instead of requiring a literal <table>, collect
    outbound hauler links in the URO content section.

    This also naturally captures the direct company website from the city page.
    """
    html = raw.decode("utf-8", errors="replace")
    soup = BeautifulSoup(html, "html.parser")
    ...
    # Companies listed in the introductory paragraph and in the actual hauler
    # rows are both valid licensed/private-hauler candidates.
    for node in heading.find_all_next():
        text = clean_text(node.get_text(" ", strip=True)) if hasattr(node, "get_text") else ""

        if node.name in {"p", "div"} and "this list is provided without endorsement" in text.lower():
            break
        ...
        href = urljoin(source.url, node["href"])
        ...
        # Exclude generic city/navigation resources.
        low = company.lower()
        if any(
            x in low
            for x in (
                "licensed private hauler",
        ...
        # Best-effort service signal from the nearest row/container text.
        context = clean_text(node.parent.get_text(" ", strip=True))
        services = []
        if re.search(r"\bX\b", context):
            # The exact X-column mapping is markup-dependent. Do not fabricate
            # which service(s) if column structure is unavailable.
            service_hint = "listed URO hauler"
        else:
            service_hint = "licensed private hauler"
...
def self_test() -> None:
    ...
    <main>
      ...
      <p>This list is provided without endorsement.</p>
    </main>
    """
